...
현재 프로젝트는 다음과 같은 진행 상황을 보이고 있습니다:

### ✅ 완료된 항목
| **단계**                 | **상태** | **설명**                                           |
|--------------------------|---------|-----------------------------------------------------|
| 환경 설정                | 완료     | GitHub 저장소 설정, OpenAI API 키 구성 완료          |
| GitHub Action 통합       | 완료     | PR에 자동으로 코드 리뷰 코멘트 게시 구현             |
| 코드 리뷰 기본 기능      | 완료     | OpenAI API 연동, 코드 리뷰 생성, 코멘트 게시 기능    |
| 인라인 코멘트 개선       | 완료     | 파일 라인별 상세 코멘트 기능 구현                    |
| 중복 코멘트 관리         | 완료     | 이전 리뷰 표시 및 관리 기능 구현                     |

### 🔄 진행 중인 항목
| **단계**                 | **상태** | **설명**                                           |
|--------------------------|---------|-----------------------------------------------------|
| 코드 리뷰 품질 개선      | 진행 중  | 더 상세하고 정확한 리뷰 코멘트 생성 최적화 중        |
| 테스트 및 문서화         | 진행 중  | 다양한 코드베이스에서 테스트 및 피드백 수집 중       |

### ❌ 향후 구현 예정 항목
| **단계**                 | **상태** | **설명**                                           |
|--------------------------|---------|-----------------------------------------------------|
| MCP 서버/클라이언트 구현 | 예정     | MCP 프로토콜 기반 서버/클라이언트 아키텍처 구현      |
| A2A 에이전트 통신        | 예정     | 에이전트 간 통신 프로토콜 구현 및 테스트             |
| 다중 레포지토리 지원     | 예정     | 다양한 GitHub 레포지토리에서 사용 가능한 확장성 추가 |

## 프로덕션 환경 로드맵

현재 학습용 프로젝트를 프로덕션 환경에서 사용 가능한 서비스로 확장하기 위한 로드맵은 다음과 같습니다:

### 1단계: 서비스 인프라 구축 (예상 기간: 2-3주)
- **독립 서버 배포**: AWS EC2 또는 Digital Ocean에 MCP 서버 배포
- **데이터베이스 설정**: 리뷰 히스토리 및 설정 저장용 MongoDB/PostgreSQL 구축
- **GitHub App 등록**: 다중 레포지토리 접근을 위한 OAuth 앱 등록

### 2단계: 기능 확장 (예상 기간: 3-4주)
- **대시보드 개발**: 설정 및 리뷰 통계 관리용 웹 인터페이스
- **고급 리뷰 기능**: 코드 품질 점수화, 보안 취약점 집중 탐지
- **사용자 정의 규칙**: 레포지토리별 맞춤 리뷰 규칙 설정 기능

### 3단계: 확장성 및 안정성 (예상 기간: 2-3주)
- **부하 테스트**: 다중 동시 리뷰 요청 처리 테스트
- **분산 아키텍처**: 필요시 서버리스 또는 컨테이너 기반 확장
- **모니터링 시스템**: API 사용량, 오류율, 응답 시간 실시간 모니터링

### 예상 비용 (프로덕션)
| **항목**            | **예상 월 비용** | **설명**                                      |
|---------------------|-----------------|------------------------------------------------|
| 서버 호스팅         | $5-20           | AWS EC2 t2.micro~t3.small 또는 동급 서비스      |
| 데이터베이스        | $0-15           | 무료 티어 사용 또는 소규모 관리형 데이터베이스   |
| OpenAI API          | $10-50          | 사용량 기준, 월 500-2500회 리뷰 가정            |
| 기타 서비스         | $0-10           | 로깅, 모니터링, CDN 등 선택적 서비스            |
| **총 예상 비용**    | **$15-95**      | 사용량과 선택한 서비스에 따라 변동              |

실제 서비스 배포 시 초기에는 최소 비용으로 시작하여 사용량에 따라 확장하는 방식을 추천합니다. AWS Free Tier나 GitHub Student Developer Pack 등을 활용하면 초기 비용을 더욱 절감할 수 있습니다.
...
## 결론
CodeSage 경량 프로젝트는 MCP와 A2A를 중심으로 한 스터디에 최적화된 솔루션입니다. MCP를 통해 AI와 코드 데이터를 연결하고, A2A로 에이전트 협력을 학습하며, 소규모 팀이 예산 없이 실질적인 경험을 쌓을 수 있습니다. 현재 프로젝트는 기본 코드 리뷰 기능을 성공적으로 구현했으며, 향후 MCP와 A2A 구현 및 프로덕션 환경으로의 확장을 계획하고 있습니다.
